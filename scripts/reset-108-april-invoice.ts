 ...
 *
 * 動作:
 * 1. 108顧客の2026年4月分Invoiceレコードを特定
 * 2. その請求書に紐付く納品(deliveryIds)のステータスを INVOICED → DELIVERED に戻す
 * 3. Invoiceレコードを削除
 *
 * 使用方法:
 *   ドライラン (確認のみ): npx tsx scripts/reset-108-april-invoice.ts
 *   実行             : npx tsx scripts/reset-108-april-invoice.ts --execute
 *
 * 注意:
 * - Google Sheetsの請求書タブはそのまま残ります（ユーザーが手動削除）
 * - 実行後は、UIの請求書作成画面から2026年4月を選んで再発行してください
 *   （329b707 の修正で合計金額がスプレッドシート上部に表示されるようになっています）
 */
...
async function main() {
  console.log('🔧 108 4月分請求書 破棄スクリプト');
  console.log(`モード: ${EXECUTE ? '🚨 実行 (--execute)' : '👀 ドライラン'}`);
  console.log('='.repeat(60));

  // 1. 108顧客を検索（社名は漢数字表記「一〇八」）
  const customers = await prisma.customer.findMany({
    where: {
      companyName: { contains: '一〇八' },
    },
  });

  if (customers.length === 0) {
    console.log('❌ 108顧客が見つかりません。中断します。');
    return;
  }
  ...
  if (!EXECUTE) {
    console.log('\n👀 ドライランのため変更は行いません。');
    console.log('🚀 実行する場合: npx tsx scripts/reset-108-april-invoice.ts --execute');
    return;
  }
