/**
 * 新しい請求書テンプレートをGoogle Sheets APIで作成するスクリプト
 * PDFデザインに基づいた9列構造の請求書テンプレート
 */
 ...
 * 新しい請求書テンプレートを作成
 */
async function createNewInvoiceTemplate() {
  ...
  requests.push({
    repeatCell: {
      ...
      cell: {
        userEnteredValue: {
          stringValue: '請求番号: {{invoiceNumber}}',
        },
        userEnteredFormat: {
          textFormat: {
            fontSize: 10,
  ...
  // 振込先情報ブロック
  const bankInfoStartRow = totalStartRow + 4;

  requests.push({
    repeatCell: {
      range: {
        sheetId: sheetId,
        startRowIndex: bankInfoStartRow,
        endRowIndex: bankInfoStartRow + 1,
        startColumnIndex: 0,
        endColumnIndex: 1,
      },
      cell: {
  ...
  bankInfoLines.forEach((line, index) => {
    requests.push({
      repeatCell: {
        range: {
          sheetId: sheetId,
          startRowIndex: bankInfoStartRow + 1 + index,
          endRowIndex: bankInfoStartRow + 2 + index,
          startColumnIndex: 0,
          endColumnIndex: 1,
        },
        cell: {
  ...
  console.log(`   GOOGLE_SHEETS_NEW_INVOICE_TEMPLATE_SHEET_ID="${spreadsheetId}"`);

  return spreadsheetId;
}

// スクリプト実行
createNewInvoiceTemplate()
  .then(() => {
    console.log('\n✅ 処理が正常に完了しました');
    process.exit(0);
  })
