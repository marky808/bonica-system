...
# 808社 進捗管理ルール（CLAUDE.mdに追加）

## 進捗管理の基本方針

すべてのプロジェクトで `PROGRESS.md` を使用して進捗を管理する。
セッション間の「記憶喪失」を防ぎ、作業の継続性を確保する。

---

## 作業終了時の必須アクション

以下のキーワードが出たら、**作業を停止し PROGRESS.md を更新すること**：
...
1. **本日の作業内容**（何をやったか、箇条書き）
2. **完了した項目**（チェックリストにチェックを入れる）
3. **次回やるべきこと**（優先順位付きで明確に）
4. **注意点・課題**（ハマったポイント、未解決の問題）
5. **更新日時**
...
「続きをやって」「再開」「前回の続き」と言われたら：

1. **まず PROGRESS.md を読む**
2. 前回の状況を簡潔に報告する
3. 「次回やるべきこと」から作業を開始する

---

## 新規プロジェクト開始時

プロジェクト開始時に `PROGRESS.md` を作成し、以下を記載：

1. プロジェクト概要
2. 機能要件のチェックリスト（全項目を `- [ ]` で列挙）
3. 技術スタック
4. 注意事項
...
既存リポジトリに PROGRESS.md がない場合：

1. リポジトリ構造を調査
2. 現状を把握してPROGRESS.mdを作成
3. 実装済み・未実装を整理してチェックリスト化
